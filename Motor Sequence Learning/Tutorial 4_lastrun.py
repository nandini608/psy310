...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\nandi\\OneDrive\\Desktop\\Ongoing Subjects\\PSY310\\Motor Sequence Learning\\Tutorial 4_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    fixation2 = visual.ShapeStim(
        win=win, name='fixation2', vertices='cross',units='pix', 
        size=(10, 10),
        ori=0.0, pos=(0, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor=[1.0000, 1.0000, 1.0000], fillColor=[1.0000, 1.0000, 1.0000],
        opacity=None, depth=0.0, interpolate=True)
    ...
    line2_2 = visual.Rect(
        ...
        opacity=None, depth=-2.0, interpolate=True)
    line3_3 = visual.Rect(
        win=win, name='line3_3',units='pix', 
        width=(100, 10)[0], height=(100, 10)[1],
        ori=90.0, pos=(50, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-3.0, interpolate=True)
    line4_4 = visual.Rect(
        win=win, name='line4_4',units='pix', 
        width=(100, 10)[0], height=(100, 10)[1],
        ori=90.0, pos=(150, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-4.0, interpolate=True)
    ...
    for thisTrial_2 in trials_2:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('trial2.started', globalClock.getTime(format='float'))
        ...
        _key_resp2_allKeys = []
        # keep track of which components have finished
        trial2Components = [fixation2, line1_2, line2_2, line3_3, line4_4, probe2, key_resp2]
        ...
        while continueRoutine:
            ...
            # if line3_3 is starting this frame...
            if line3_3.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line3_3.frameNStart = frameN  # exact frame index
                line3_3.tStart = t  # local t and not account for scr refresh
                line3_3.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line3_3, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line3_3.started')
                # update status
                line3_3.status = STARTED
                line3_3.setAutoDraw(True)
            
            # if line3_3 is active this frame...
            if line3_3.status == STARTED:
                # update params
                pass
            
            # *line4_4* updates
            
            # if line4_4 is starting this frame...
            if line4_4.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line4_4.frameNStart = frameN  # exact frame index
                line4_4.tStart = t  # local t and not account for scr refresh
                line4_4.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line4_4, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line4_4.started')
                # update status
                line4_4.status = STARTED
                line4_4.setAutoDraw(True)
            
            # if line4_4 is active this frame...
            if line4_4.status == STARTED:
                # update params
                pass
        ...
        for thisComponent in trial2Components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('trial2.stopped', globalClock.getTime(format='float'))
        # check responses
